...
const JWT_SECRET_KEY = '9e798d13977da2c82c915092d73b36ed8f9a513066202ce299348a92a32560d2';
const app = express();
const PORT = process.env.PORT || 5000;

app.use(express.json());
const uri = 'mongodb+srv://user2:<password>@cluster.qtqy1u4.mongodb.net/?retryWrites=true&w=majority&appName=Cluster';
const password = 'minamino';
...
app.post('/api/admin/register', async (req, res) => {
    try {
        ...
        const existingAdmin = await client.db('Pollquestionapp').collection('pollquestioncollection').findOne({ username });
        if (existingAdmin) {
        ...
        await client.db('Pollquestionapp').collection('pollquestioncollection').insertOne({ name, username, email, password: hashedPassword });
...
app.post('/api/admin/login', async (req, res) => {
    try {
        ...
        const admin = await client.db('Pollquestionapp').collection('pollquestioncollection').findOne({ username });
        if (!admin) {
...
app.put('/api/admin/change-password', async (req, res) => {
    try {
        ...
        await client.db('Pollquestionapp').collection('pollquestioncollection').updateOne(
            { username },
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const existingUser = await client.db('Pollquestionapp').collection('users').findOne({ username });
        if (existingUser) {
        ...
        await client.db('Pollquestionapp').collection('users').insertOne({ name, username, email, password: hashedPassword});
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        const user = await client.db('Pollquestionapp').collection('users').findOne({ username });
        if (!user) {
            return res.status(400).json({ message: 'Invalid username or password' });
        }
...
app.put('/api/change-password', async (req, res) => {
    try {
        ...
        await client.db('Pollquestionapp').collection('users').updateOne(
            { username },
            { $set: { password: hashedNewPassword } }
        );
...
app.post('/api/reset-password-request', async (req, res) => {
    try {
        ...
        const user = await client.db('Pollquestionapp').collection('users').findOne({ username, email, yearOfBirth });
        if (!user) {
